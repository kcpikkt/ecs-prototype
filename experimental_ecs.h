...
namespace proto{
    namespace ecs{
        ...
        template<typename ...CTs> struct ECS {
            ...
            constexpr static Entity create_entity(){
            ...
            template<typename C>
            constexpr static size_t create_component(Entity e){ 
            ...
            constexpr static void delete_entity(Entity e){ 
                if(e.id() == 0){return;}
            ...
            template<typename C>
            constexpr static void delete_component(Entity e){ 
                if(dmux<C>(e) != 0){
                    get_component_arr<C>()[ dmux<C>(e) ].parent_entity._index = 0;
                    get_component_arr<C>().free( dmux<C>(e) );
                    dmux_mut<C>(e) = 0;
                }
            }
            template<typename C>
            constexpr static bool has_component(Entity e){
                return (dmux<C>(e) != 0);
            }
            template<typename C>
            constexpr static C& get_component_mut(Entity e){ 
...
#if 0
            static void print_entity(entity_t id){
                print("e(",id,", ", entity_data[id].last_gen,") ");
                for(size_t i=0; i<components_count; i++ ){
                    print(" ", entity_data[id].components_indices[i]);
                }
                print('\n');
            }
            void printecs(){
